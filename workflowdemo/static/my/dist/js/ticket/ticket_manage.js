...
var table = $('#ticket_table').DataTable({
  ...
  ajax: function (data, callback, settings) {
    ...
    $.ajax({
      type: "GET",
      url: "http://" + hostserverip + "/workflow/alltickets",
      cache: false,  //禁用缓存
      data: param,  //传入组装的参数
      dataType: "json", 
  ...
  columns: [
      ...
      {render: function(data, type, full){
       ...
       var detail_link = '<a href ="/workflow/ticket/' + full.id + '"' +  '>详情</a>';
        return '<div>' + detail_link  +  '</div>'}}
  ]
...
$( document ).ready(function() {
    ...
    $.ajax({
      type: "GET",
      url: "http://" + hostserverip + "/workflow/user_admin",
      cache: false,  //禁用缓存
      dataType: "json",
      success: function (ret) {
        var result = ret.value;
...
$('#search_creator').select2({
    ...
    ajax: {
      url: "http://127.0.0.1:10000/api/v1.0/accounts/users",
      delay: 300,
      dataType: 'jsonp',
  ...
  swal({
    ...
    .then(function(willDelete){
      if (willDelete) {
        ...
        $.ajax({
        type: "DELETE",
        url: "http://127.0.0.1:10000/api/v1.0/tickets/" + ticketId,
        cache: false,  //禁用缓存
        dataType: "jsonp",
